...
macro_rules! def_id {
    ...
    ($struct_name:ident, $prefix:literal $(| $alt_prefix:literal)* $(, { $generate_hint:tt })?) => {
        ...
        impl $struct_name {
            ...
            /// The valid prefixes of the id type (e.g. `ch_` for a `ChargeId`).
            #[inline(always)]
...
def_id!(AuthorisationId, "auth_");
def_id!(ApplePayId, "apmd_");
def_id!(BankAccountId, "ba_");
def_id!(CardId, "card_");
def_id!(ChargeId, "ch_"); 
def_id!(CustomerId, "cus_");
def_id!(DepositId, "dpo_");
def_id!(DisputeId, "dis_");
def_id!(EventId, "evt_");
def_id!(FileId, "file_");
def_id!(MerchantId, "mrch_");
def_id!(PaymentSourceId, "ps_");
def_id!(PlanId, "plan_");
def_id!(RecipientId, "rp_");
def_id!(RefundId, "rf_");
def_id!(SessionId, "se_");
def_id!(SubscriptionId, "sub_");
def_id!(TransferId, "tfer_");
def_id!(WebhookEndpointId, "whe_");
def_id!(WebhookId, "whr_");
...
mod tests {
    ...
    fn test_parse_charge() {
        ...
        if let Err(err) = bad_parse {
            assert_eq!(
                format!("{}", err),
                "invalid `ChargeId`, expected id to start with \"ch_\""
            );
        }
    }
    ...
    fn test_parse_session() {
        assert!("se_123".parse::<SessionId>().is_ok());
        let bad_parse = "zz_123".parse::<SessionId>();
        assert!(bad_parse.is_err());
        if let Err(err) = bad_parse {
            assert_eq!(
                format!("{}", err),
                "invalid `SessionId`, expected id to start with \"se_\""
            );
        }
    }
}
